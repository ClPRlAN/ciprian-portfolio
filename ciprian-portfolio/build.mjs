...
function card({ kicker, title, description, href, tags=[], inverse=false, cover='' }) {
  const media = cover ? `<div class="card-media"><img src="${cover}" alt="" loading="lazy"></div>` : '';
  return `<a class="card reveal${inverse?' card-inverse':''}${cover?' card-with-media':''}" href="${href}">${media}<div class="card-content"><div class="card-kicker">${esc(kicker)}</div><h3>${esc(title)}</h3>${description?`<p>${esc(description)}</p>`:''}<div class="card-bottom"><div class="tags">${tags.slice(0,3).map(x=>`<span class="tag">${esc(x)}</span>`).join('')}</div><span class="arrow">↗</span></div></div></a>`;
}
function sectionCards(lang, id, title, items, hrefAll, kind) {
  ...
  const cards = items.slice(0,3).map(item => {
    ...
    const desc = item.description ? (typeof item.description==='string'?item.description:item.description[lang]) : (item.abstract ? item.abstract[lang] : (item.issuer || ''));
    let href = hrefAll;
...
function presentationsPage(lang) {
  const t=ui[lang];
  const items=presentations.map(p=>card({kicker:p.date,title:p.title,description:p.description[lang],href:routeFor(lang,`/presentations/${p.slug}`),cover:p.cover||''})).join('');
...
function researchPage(lang) {
  const t=ui[lang];
  const items=research.map(r=>`<article class="research-item reveal"><div class="research-cover"><img src="${r.cover}" alt="${esc(r.title)} cover" loading="lazy"></div><div class="research-copy"><div class="research-meta"><span>${esc(r.year)}</span><span>${esc(r.institution||'')}</span></div><h3>${esc(r.title)}</h3><p>${esc(r.abstract[lang])}</p><div class="tags" style="margin-top:14px"><span class="tag">${esc(r.authors)}</span>${r.topic!=='—'?`<span class="tag">${esc(r.topic)}</span>`:''}</div></div><div class="research-action">${r.pdf?`<a class="button" href="${r.pdf}" target="_blank" rel="noopener">${esc(t.common.openPdf)} ↗</a>`:`<span class="tag">PDF soon</span>`}</div></article>`).join('');
...
function certificationsPage(lang) {
  const t=ui[lang];
  const items=certifications.map(c=>`<a class="cert reveal" href="${c.pdf}" target="_blank" rel="noopener"><div class="cert-media"><img src="${c.cover}" alt="${esc(c.title)} certificate preview" loading="lazy"></div><div class="cert-body"><span class="cert-year">${esc(c.year)}${c.validUntil?` · ${esc(t.common.validUntil)} ${esc(c.validUntil)}`:''}</span><h3>${esc(c.title)}</h3><p>${esc(c.issuer)}</p><span class="cert-link">${esc(t.common.viewCertificate)} ↗</span></div></a>`).join('');
